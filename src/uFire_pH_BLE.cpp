...
float uFire_pH_BLE::measurepH() {
  float ph = ISE_pH::measurepH();
  String spH = String(ph);

  pmV_Characteristic->setValue(spH.c_str());
  pmV_Characteristic->notify();
  return ph;
}

float uFire_pH_BLE::measurepH(float temp) {
  float ph = ISE_pH::measurepH(temp);
  String spH = String(ph);
...
float uFire_pH_BLE::measureTemp() {
  float temp = ISE_pH::measureTemp();
